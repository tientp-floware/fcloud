  ...
  if [ -f $XdebugFile ]; then
    ...
    if [ -f $XdebugFile ]; then
        ...
        if grep -q xdebug.remote_enable "$XdebugFile"; then
            ...
            echo "xdebug.mode=debug,develop,trace, profile" >> $XdebugFile
            echo "xdebug.${XDEBUG_CONFIG}" >> $XdebugFile
            echo "xdebug.client_port=9000" >> $XdebugFile
            echo "xdebug.start_with_request=yes" >> $XdebugFile
            echo "xdebug.output_dir=/var/www/html/data" >> $XdebugFile
